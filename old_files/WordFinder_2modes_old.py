...
def find_pattern_words(
    ...
    Within each mode (start/end/middle), multiple patterns are OR-ed:
    the word must match AT LEAST ONE pattern in each non-empty mode group.
    Across modes (start vs end vs middle), the constraint is AND:
    all non-empty mode groups must be satisfied.
    """
    # Filter out empty patterns
    start_pats = [p for p in start_patterns if p["seq"]]
    end_pats = [p for p in end_patterns if p["seq"]]
    mid_pats = [p for p in middle_patterns if p["seq"]]
    ...
    for word in words_list:
        ...
        # start: word must match at least one start pattern (OR within group)
        if start_pats:
            if not any(_pat_matches_start(word, p, language) for p in start_pats):
                continue

        # end: word must match at least one end pattern
        if end_pats:
            if not any(_pat_matches_end(word, p, language) for p in end_pats):
                continue

        # middle: word must match at least one middle pattern
        if mid_pats:
            if not any(_pat_matches_middle(word, p, language) for p in mid_pats):
                continue
...
Expanded matching (Ctrl+Space in a slot):
  Normally, "οσ" matches only the literal string "οσ".
  Pressing Ctrl+Space on a slot expands it to match all accent/case variants,
  e.g. "οόΟΌσΣς" for "οσ". Press Ctrl+Space again to collapse.
...
═══════════════ Common Controls ═══════════════
  ...
  • Space — toggle between Slot and All scope.
...
def render_header():
    ...
    # Two-line title: "Word Finder" on first line, "[mode]" on second
    mode_label = (
    ...
    # Key binding hints (updated bindings per item 9)
    hints1 = (
        "  ·  Backspace = Clear  ·  Enter = Search  ·  ← → = Navigate  ·  ↑ ↓ = Mode"
        "  ·  Tab = Letter Match/Pattern Hunt  ·  Space = Slot/All  ·  "
    )
    hints2 = (
    ...
    blit_text(
        screen, hints1, FONT_SM, MUTED, PAD + 200, 1 / 4 * H_HEADER, anchor="midleft"
    )
    blit_text(
        screen, hints2, FONT_SM, MUTED, PAD + 200, 3 / 4 * H_HEADER, anchor="midleft"
    )
...
def render_workspace_lm(mouse_pos):
    ...
    btn_h = 30
    summary_btn = pygame.Rect(PAD, WORKSPACE_Y, 150, btn_h)
    draw_button(
        screen,
        summary_btn,
        "Slots Review",
    ...
    return slot_w, slot_h, sx, ty, gap, WORKSPACE_Y + PAD + 170, summary_btn
...
def render_workspace_ph(mouse_pos):
    ...
    mode_styles = {
        ...
        "middle": ("MIDDLE", PURPLE_BG, PURPLE_BDR, PURPLE),
    }
    ...
    for mode_name in ["start", "end", "middle"]:
        ...
        for i in range(num_slots):
            ...
            if cell.collidepoint(mouse_pos):
                disp_full = (
                    expand_sequence(seq, state.language)
                    if expanded
                    else (seq or "empty")
                )
    ...
    return WORKSPACE_Y + PAD + 170, summary_btn, ph_slot_rects
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Space = toggle language or expand if ctrl+space
                elif event.key == pygame.K_SPACE:
                    if state.finder_mode == "pattern_hunt" and (
                        event.mod & pygame.KMOD_CTRL
                    ):
                        # Ctrl+Space in PH = toggle expand
                        ph_toggle_expand()
                        refresh_summary_window()
                    else:
                        if state.finder_mode == "letter_match":
